//
//  MKBXDPowerSaveTriggerTimeCell.m
...
@implementation MKBXDPowerSaveTriggerTimeCellModel
@end

@interface MKBXDPowerSaveTriggerTimeCell ()
...
@implementation MKBXDPowerSaveTriggerTimeCell

+ (MKBXDPowerSaveTriggerTimeCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXDPowerSaveTriggerTimeCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXDPowerSaveTriggerTimeCellIdenty"];
    if (!cell) {
        cell = [[MKBXDPowerSaveTriggerTimeCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXDPowerSaveTriggerTimeCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKBXDPowerSaveTriggerTimeCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXDPowerSaveTriggerTimeCellModel.class]) {
        return;
    }
...
- (MKTextField *)textField {
    if (!_textField) {
        ...
        _textField.textChangedBlock = ^(NSString * _Nonnull text) {
            ...
            [self setNeedsLayout];
            if ([self.delegate respondsToSelector:@selector(bxd_powerSaveTriggerTimeChanged:)]) {
