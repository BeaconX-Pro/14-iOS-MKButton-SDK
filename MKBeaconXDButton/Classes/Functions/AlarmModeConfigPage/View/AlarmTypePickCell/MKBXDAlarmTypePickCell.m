...
@implementation MKBXDAlarmTypePickCellModel
@end

@interface MKBXDAlarmTypePickCell ()<UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation MKBXDAlarmTypePickCell

+ (MKBXDAlarmTypePickCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXDAlarmTypePickCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXDAlarmTypePickCellIdenty"];
    if (!cell) {
        cell = [[MKBXDAlarmTypePickCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXDAlarmTypePickCellIdenty"];
    }
    return cell;
}
...
- (void)layoutSubviews{
    ...
    [self.typeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.mas_equalTo(self.pickerView.mas_left).mas_offset(-10.f);
        make.centerY.mas_equalTo(self.backView.mas_centerY);
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    self.triggerAlarmType = row;
    [self.pickerView reloadAllComponents];
    if ([self.delegate respondsToSelector:@selector(bxd_triggerAlarmTypeChanged:)]) {
        [self.delegate bxd_triggerAlarmTypeChanged:row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKBXDAlarmTypePickCellModel *)dataModel {
    _dataModel = nil;
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBeaconXDButton", @"MKBXDAlarmTypePickCell", @"bxd_slottriggerAlarmType.png");
    }
    return _leftIcon;
}
...
- (UILabel *)typeLabel{
    if (!_typeLabel) {
        ...
        _typeLabel.text = @"Alarm notification type";
    }
    return _typeLabel;
}
