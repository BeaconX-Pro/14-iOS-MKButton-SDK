...
@implementation MKBXDSlotUIDCellModel
@end

@interface MKBXDSlotUIDCell ()
...
@implementation MKBXDSlotUIDCell

+ (MKBXDSlotUIDCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXDSlotUIDCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXDSlotUIDCellIdenty"];
    if (!cell) {
        cell = [[MKBXDSlotUIDCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXDSlotUIDCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKBXDSlotUIDCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXDSlotUIDCellModel.class]) {
        return;
    }
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBeaconXDButton", @"MKBXDSlotUIDCell", @"bxd_slotAdvContent.png");
    }
    return _leftIcon;
}
...
- (MKTextField *)nameTextField {
    if (!_nameTextField) {
        ...
        _nameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxd_advContent_namespaceIDChanged:)]) {
...
- (MKTextField *)instanceTextField {
    if (!_instanceTextField) {
        ...
        _instanceTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxd_advContent_instanceIDChanged:)]) {
