...
@implementation MKBXDSlotFramePickCellModel
@end

@interface MKBXDSlotFramePickCell ()<UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation MKBXDSlotFramePickCell

+ (MKBXDSlotFramePickCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXDSlotFramePickCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXDSlotFramePickCellIdenty"];
    if (!cell) {
        cell = [[MKBXDSlotFramePickCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXDSlotFramePickCellIdenty"];
    }
    return cell;
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    ...
    if ([self.delegate respondsToSelector:@selector(bxd_slotFrameTypeChanged:)]) {
        [self.delegate bxd_slotFrameTypeChanged:row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKBXDSlotFramePickCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXDSlotFramePickCellModel.class]) {
        return;
    }
    [self.pickerView reloadAllComponents];
    self.frameType = _dataModel.frameType;
    [self.pickerView selectRow:_dataModel.frameType inComponent:0 animated:YES];
}
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBeaconXDButton", @"MKBXDSlotFramePickCell", @"bxd_slotFrameType.png");
    }
    return _leftIcon;
}
...
- (NSMutableArray *)dataList {
    if (!_dataList) {
        _dataList = [NSMutableArray arrayWithObjects:@"Alarm info",@"UID",@"iBeacon", nil];
    }
    return _dataList;
}
