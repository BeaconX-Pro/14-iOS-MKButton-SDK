...
@implementation MKBXDSlotBeaconCellModel
@end

@interface MKBXDSlotBeaconCell ()
...
@implementation MKBXDSlotBeaconCell

+ (MKBXDSlotBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXDSlotBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXDSlotBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKBXDSlotBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXDSlotBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKBXDSlotBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXDSlotBeaconCellModel.class]) {
        return;
    }
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBeaconXDButton", @"MKBXDSlotBeaconCell", @"bxd_slotAdvContent.png");
    }
    return _leftIcon;
}
...
- (MKTextField *)majorTextField {
    if (!_majorTextField) {
        ...
        _majorTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxd_advContent_majorChanged:)]) {
...
- (MKTextField *)minorTextField {
    if (!_minorTextField) {
        ...
        _minorTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxd_advContent_minorChanged:)]) {
...
- (MKTextField *)uuidTextField {
    if (!_uuidTextField) {
        ...
        _uuidTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxd_advContent_uuidChanged:)]) {
