...
@interface MKBXDScanDeviceDataCell ()
...
@implementation MKBXDScanDeviceDataCell

+ (MKBXDScanDeviceDataCell *)initCellWithTableView:(UITableView *)tableView{
    MKBXDScanDeviceDataCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXDScanDeviceDataCellIdenty"];
    if (!cell) {
        cell = [[MKBXDScanDeviceDataCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXDScanDeviceDataCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed{
    ...
    if ([self.delegate respondsToSelector:@selector(mk_bxd_connectPeripheral:)]) {
        [self.delegate mk_bxd_connectPeripheral:self.dataModel.peripheral];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKBXDScanDataModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXDScanDataModel.class]) {
        return;
    }
...
//    self.timeLabel.text = _dataModel.displayTime;
    ...
    if (!ValidStr(_dataModel.battery)) {
        ...
        NSString *unitString = ([_dataModel.battery integerValue] > 100) ? @"mV" : @"%";
        self.batteryLabel.text = [_dataModel.battery stringByAppendingString:unitString];
    }
...
- (UIImageView *)rssiIcon{
    if (!_rssiIcon) {
        ...
        _rssiIcon.image = LOADICON(@"MKBeaconXDButton", @"MKBXDScanDeviceDataCell", @"bxd_signalIcon.png");
    }
    return _rssiIcon;
}
...
- (UIImageView *)batteryIcon{
    if (!_batteryIcon) {
        ...
        _batteryIcon.image = LOADICON(@"MKBeaconXDButton", @"MKBXDScanDeviceDataCell", @"bxd_batteryHighest.png");
    }
    return _batteryIcon;
}
